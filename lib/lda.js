...
//
var process = function(sentences, numberOfTopics, numberOfTermsPerTopic, languages, alphaValue, betaValue, specialCharacters, num_iterations) {
    ...
    // special characters to retain in words
    ...
    for(var i=0;i<sentences.length;i++) {
        ...
        for(var wc=0;wc<words.length;wc++) {
            var re = new RegExp("[^a-z\'A-Z0-9\u00C0-\u00ff" + specialCharacters + " ]+", "g");
            var w=words[wc].toLowerCase().replace(re, '');
            var wStemmed = w; //w.startsWith('u') ? w : stem(w);
    ...
    lda.configure(documents,V,num_iterations, 30, 100, 10);
